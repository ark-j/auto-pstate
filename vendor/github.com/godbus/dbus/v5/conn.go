...
func ConnectSystemBus(opts ...ConnOption) (*Conn, error) {
	addr := getSystemBusPlatformAddress()
	log.Println("2. address of system bus:", addr)
...
func Connect(address string, opts ...ConnOption) (*Conn, error) {
	log.Println("3. client connection to socket")
	conn, err := Dial(address, opts...)
	if err != nil {
		return nil, err
	}
	log.Println("4. Performing auth")
...
func (conn *Conn) Connected() bool {
	log.Println("checking if system connected:", conn.ctx.Err() == nil)
